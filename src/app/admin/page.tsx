...
export default async function AdminPage({
  ...
  //   UNVERIFIED → live on the map but no verified badge yet (legacy / manual)
  //   VERIFIED   → live + team-confirmed
  //   REJECTED   → unpublished
  // PENDING is the default landing tab — it's the queue that needs work.
  const filter = sp.status?.toUpperCase();
  const allowed = ["PENDING", "UNVERIFIED", "VERIFIED", "REJECTED"] as const;
  ...
  const [pendingCount, unverifiedCount, verifiedCount, rejectedCount] =
    await Promise.all([
  ...
  return (
    <div className="mx-auto max-w-6xl px-4 sm:px-6 py-10">
      ...
      <nav className="mt-5 flex flex-wrap gap-2 text-sm">
        {(["PENDING", "UNVERIFIED", "VERIFIED", "REJECTED"] as const).map((s) => {
          const active = s === tab;
          return (
            <a
              key={s}
